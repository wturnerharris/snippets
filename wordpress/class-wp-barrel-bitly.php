...
class BarrelBitly {
	...
	function __construct(){
		if ( class_exists('Bitly') ){
			// instantiate bitly class on class property
			$this->bitly = new Bitly( array(
				'bitlyKey' => 'R_49bb81bd7dd769cc42f6a2b1c7de5a6e',
				'bitlyLogin' => 'barrelny',
				'bitlyClientId' => 'e26e310198a640869678b2bcd3f0657a51baff9f',
				'bitlySecret' => 'f8d63e53a17438f67140bae193851a3054e724a5'
			) );
		}
		
		if ( is_null($bitly) ) return false; 
		
		// replace filters with custom make_clickable code (based on wordpress version)
		add_filter( 'content_save_pre', array(&$this, 'make_clickable_shorten'), 12 );
		add_filter( 'excerpt_save_pre ', array(&$this, 'make_clickable_shorten'), 12 );
		add_filter( 'comment_save_pre ', array(&$this, 'make_clickable_shorten'), 12 );
	}
	 ...
	 */
	function make_clickable_shorten( $text ) {
		$r = '';
			...
			if ( 10000 < strlen( $piece ) ) {
				...
				foreach ( _split_str_by_whitespace( $piece, 2100 ) as $chunk ) { // 2100: Extra room for scheme and leading and trailing paretheses
					if ( 2101 < strlen( $chunk ) ) {
						...
						$r .= $this->make_clickable_shorten( $chunk );
					}
				}
			} else {
				...
				~xS'; // The regex is a non-anchored pattern and does not have a single fixed starting character.
					  ...
					  // ONLY perform our filtration on regular urls
				$ret = preg_replace_callback( $url_clickable, array(&$this, '_make_url_clickable_cb_shorten'), $ret );
	 ...
	 */
	function _make_url_clickable_cb_shorten($matches) {
		$url = $matches[2];
		...
		$url = esc_url($url);
		$bit = $this->bitly->bitly_v3_shorten($url);
		$url = $bit['url'];
		if ( empty($url) )
			return $matches[0];
		return $matches[1] . "<a href=\"$url\" rel=\"nofollow\" title=\"{$bit['long_url']}\">$url</a>" . $suffix;
	}
